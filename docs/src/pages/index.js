...
function Feature({imageUrl, title, description, imageData}) {
  const imgUrl = useBaseUrl(imageUrl);
  return (
    <div className={classnames('col col--4', styles.feature)}>
      {imgUrl && (
        <div className="text--center">
          <img className={styles.featureImage} src={imageData || imgUrl} alt={title} />
        </div>
      )}
      <h3>{title}</h3>
      <p>{description}</p>
...
function Home() {
  ...
  return (
    <Layout
      ...
      description="XMars UI - HTML, CSS, React">
